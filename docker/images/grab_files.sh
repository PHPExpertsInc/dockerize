...
if [ -d "$1" ]; then
    ...
    for each in $(find "$1" -name \*.so\* -type f); do 
        if ! deps=$(ldd_deps "$each"); then
            echo "Error: Aborting due to unresolvable dependencies for $each." >&2
            exit 3
        fi
        if [ -n "$deps" ]; then cp -v $deps /tmp/distroless/usr/lib; fi
    done
...
if [ -x "$1" ]; then
    ...
    if [ -n "$deps" ]; then cp -vf $deps /tmp/distroless/usr/lib/; fi
fi
